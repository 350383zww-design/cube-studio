...
`;
	...
	return (
		<Space className="tablebox" direction="vertical" size={16}>
			<Modal
				width={1000}
				maskClosable={false}
				centered={true}
			...
			{
				props.titleNode || props.buttonNode || !props.cancelExportData ? <Row justify="space-between" align="middle" gutter={[16, 16]}>
					<Col>
						<Space align="center" size={16}>{props.titleNode}</Space>
					</Col>
					<Col>
						<Space align="center" size={16}>
							{props.buttonNode}
							{props.cancelExportData ? null : (
								<Button onClick={() => setExportDataVisible(true)}>
									{'导出数据'}
								</Button>
							)}
						</Space>
